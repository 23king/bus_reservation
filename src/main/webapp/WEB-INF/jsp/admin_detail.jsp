...
<main role="main" class="container">
    ...
    <div style="margin-bottom: 50px;">
        <form name="searchForm" id="searchForm">
            <input type="hidden" name="reservDate" value="${params.reservDate}"/>
            ...
            <input type="hidden" name="bankAccountName" value="${params.bankAccountName}"/>
        </form>
        ...
        <table class="table">
            <tr>
                ...
                <th class="title" scope="col">성명</th>
                <th class="title" scope="col">좌석갯수</th>
        ...
        <table class="table">
            <tr>
                ...
                <th class="title" scope="col">버스번호</th>
                <th class="title" scope="col">좌석번호</th>
                <th class="title" scope="col">좌석상태</th>
            </tr>
            <c:forEach var="list" items="${buses}" varStatus="st">
            <tr>
                <td scope="col">${st.index}</td>
                <td scope="col">${list.busNum}</td>
                <td scope="col">${list.seatNum}</td>
                <td scope="col">${list.isTwinSeat?"동반좌석":"일반"}</td>
            </tr>
            </c:forEach>
        </table>
    </div>
    <c:if test="${travelInfo.refund != null}">
    <div>
        ...
        <table class="table">
            ...
            <tr>
                <td scope="col">${reservInfo.refund.refundAmount}원</td>
                <td scope="col">${reservInfo.refund.refundBankName}</td>
                <td scope="col">${reservInfo.refund.refundBankNo}</td>
                <td scope="col">${reservInfo.refund.refundBankAccount}</td>
                <td scope="col">
                    <select class="custom-select" id="refundStatus">
                        <option value="1" <c:if test="${reservInfo.refund.refundStatus == 1}"></c:if>>환불 신청</option>
                        <option value="2" <c:if test="${reservInfo.refund.refundStatus == 2}"></c:if>>>환불 진행중</option>
                        <option value="3" <c:if test="${reservInfo.refund.refundStatus == 3}"></c:if>>>환불 완료</option>
                    </select>
                    <button type="button" class="btn btn-success" onclick="changeRefundStatus(${reservInfo.refund.seq})">변경</button>
                </td>
            </tr>
        </table>
    </div>
        ...
        <%--<div class="bus_info">--%>
            <%--<table class="bus_info" style="width:618px;border:0;cellspacing:0;cellpadding:0;margin-top:5px;">--%>
                <%--<tbody><tr>--%>
                    ...
                    <%--&lt;%&ndash;<td style="width:31px;text-align:left;"><img src="/asset/img/seat/seat_box_03.gif" alt="" width="16" height="16"></td>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<td style="width:80px;text-align:left;"><strong class="set01">2인전용</strong></td>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<td style="width:32px;text-align:left;"><img src="/asset/img/seat/seat_box_04.gif" alt="" width="16" height="16"></td>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<td style="width:217px;text-align:left;"><strong class="set01">선택불가</strong></td>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<td style="width:26px;">&nbsp;</td>&ndash;%&gt;--%>
                <%--</tr>--%>
                <%--</tbody>--%>
            <%--</table>--%>
        <%--</div>--%>
...
<script>
    function changeRefundStatus(seq){
        ...
        $.ajax({
            type: "POST",
            url: "/api/v1/reservation/changeRefundStatus",
